...
class RBM(object):
    ...
    def get_cost_updates(self, mom=0.9, lr=0.01, persistent=None, k=1,
                         l1=0., l2=0.01):
        ...
        L1 = T.cast(l1, dtype=theano.config.floatX)*(T.abs_(self.W).sum())
        L2 = T.cast(l2, dtype=theano.config.floatX)*(self.W**2).sum()
        # weight-decay
        cost = T.mean(self.free_energy(self.input)) - T.mean(
            self.free_energy(chain_end)) + L1 + L2

        # We must not compute the gradient through the gibbs sampling
        self.gparams = T.grad(cost, self.params, consider_constant=[chain_end])
        # end-snippet-3 start-snippet-4
        # constructs the update dictionary
        # velocity_t+1 = velocity_t*mom + lr*gradient
        # params_t+1 = params - velocity_t+1
        # adaptive lr = 0.1*grad_{i}/sum_{i=1}{grad_{i}} (different param)
        Vs = [theano.shared(numpy.zeros_like(param.get_value(borrow=True),
                                             dtype=theano.config.floatX)) for param in self.params]
        Adaptive = [theano.shared(numpy.ones_like(param.get_value(borrow=True),
                                                  dtype=theano.config.floatX)) for param in self.params]

        for gparam, param, v, a in zip(self.gparams, self.params, Vs, Adaptive):
            # make sure that the learning rate is of the right dtype
            if param is self.W:
                # slow down the learning rate of Weight
                updates[a] = a + T.sqrt(gparam**2)
                updates[v] = T.cast(mom, dtype=theano.config.floatX) * v + \
                    gparam * 0.0001 / a
...
def test_rbm(dataset, datasize, record=True, l1=0., l2=0.0, mom=0.9,
    ...
    n_train_batches = int(train_set_x.get_value(
        borrow=True).shape[0] * datasize) / batch_size
...
if __name__ == '__main__':
    ...
    data = numpy.load('/home/tn00372136/nlp-lab/Rec/data_lazy_test/\
AL_all_trans.npy').tolist().tocsc().astype(theano.config.floatX)
    model = test_rbm(dataset=data, record=False,
                     datasize=0.001, training_epochs=50, batch_size=100)
