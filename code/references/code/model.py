...
class DGLFRM(Model):  # DGLFRM See paper
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, num_classes=0, **kwargs):
        ...
        self.w_gen_1 = weight_variable_glorot(
            self.hidden[self.num_hidden_layers-1], FLAGS.g_hidden, name='weight_generator_1')
        self.b_gen_1 = tf.Variable(
            tf.zeros(FLAGS.g_hidden), name='bias_generator_1')

        self.w_gen_2 = weight_variable_glorot(
            FLAGS.g_hidden, FLAGS.g_hidden/2, name='weight_generator_2')
        self.b_gen_2 = tf.Variable(
            tf.zeros(FLAGS.g_hidden/2), name='bias_generator_2')

        self.w_gen_x = weight_variable_glorot(
            self.hidden[-1], self.input_dim, name='weight_generator_x_recon')
        self.b_gen_x = tf.Variable(
            tf.zeros(self.input_dim), name='bias_generator_x_recon')

        #self.w_gen_y = weight_variable_glorot(self.hidden[-1], self.num_classes, name='weight_labels')
        #self.b_gen_y = tf.Variable(tf.zeros(self.num_classes), name='bias_labels')
    ...
    def _build(self):
        ...
        self.total_bias = entity_bias_matrix + \
            tf.tile(self.c, [self.n_samples, self.n_samples])
        ...
        # Try w/o self.pi_logit
        self.logit_post = self.pi_logit + logit(tf.exp(self.log_prior))

        # note: logsample is just logit(z_discrete), unless we've rounded
        self.z_discrete, self.z_real, _, self.y_sample = sample(
            self.z_mean, self.z_log_std, self.logit_post, None, None, FLAGS.temp_post, calc_v=False)
        self.z_discrete = tf.cond(tf.equal(self.training, tf.constant(
            False)), lambda: tf.round(self.z_discrete), lambda: self.z_discrete)

        z = tf.multiply(self.z_discrete, self.z_real)

        f = tf.nn.leaky_relu(tf.matmul(z, self.w_gen_1) +
                             self.b_gen_1, alpha=0.2)
        #f = tf.nn.dropout(f, 1-self.dropout)

        f = tf.matmul(f, self.w_gen_2) + self.b_gen_2

        self.reconstructions = InnerProductDecoder(input_dim=16,
                                                   act=lambda x: x,
                                                   logging=self.logging)(f) + tf.reshape(self.total_bias, [-1])

        self.x_hat = tf.reshape(
            tf.matmul(z, self.w_gen_x) + self.b_gen_x, [-1])

        """
        if(FLAGS.semisup_train):
            #can use final or prefinal layer or last d
            #self.pred_logits = tf.matmul(z, self.w_gen_y) + self.b_gen_y
            self.pred_logits = weight_variable_glorot(self.n_samples, self.num_classes, name='pred_logits')
            #self.to_regularize.append(self.prefinal)
            t = tf.concat([z, self.pred_logits], 1)
            print t.get_shape()
            self.x_hat = tf.reshape(tf.matmul(t, self.w_gen_x) + self.b_gen_x, [-1])
        """
